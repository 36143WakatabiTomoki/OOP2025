...
namespace Test02 {
    internal class Program {
        ...
        private static void Exercise04(int[] numbers) {
            foreach (var number in numbers.Where(n => n >= 10 && n <= 50)) {
                Console.Write(number+" ");
            }
        }
        ...
        private static void Exercise07(List<string> cities) {
            var orderCities = cities.Order().ToList();
            foreach (var citie in orderCities) {
        ...
        private static void Exercise09(List<string> cities) {
            var orderCities = cities.OrderBy(c => c.Length);
            foreach (var citie in orderCities) {
                Console.WriteLine($"{citie}：{citie.Length}文字");
            }
        }
        ...
        private static void Exercise10(List<string> cities) {
            var citieLength = cities.Where(c => c.Length == 6);
            foreach (var citie in citieLength) {
                Console.WriteLine(citie);
            }
        }
    }
